...
function render(){
  ...
  if(S.tab==='introduction'){
    ...
    h+=rEstimateSummary();
    h+=`<section id="detailed-estimates" class="est-source SectionBoard SurfaceCard"><div class="est-source-head"><h3>${t('sourceTitle')}</h3><p>${t('sourceSub')}</p></div>`;
    h+=`<div class="dash">`;
    T.forEach((o,i)=>{
      const low=o.t===mn&&o.t>0;
      h+=`<div class="dc BudgetWidget DecisionCard ${o.c} ${S.slide===i?'on':''}" onclick="goSlide(${i})">
    ...
    h+=`<div class="gt green"><span>${t('optionTotal')} A ${t('total')}${S.contOn?' (w/ '+S.contPct+'%)':''}</span><span>${fmt(T[0].t)}</span></div>`;
    h+=`</div></div></div>`;

    // SLIDE B
    h+=`<div class="csl"><div class="ocard"><div class="ohdr blue"><div class="tr"><h2>${t('slideB')}</h2><span class="pbadge blue">${fmt(T[1].t)}</span></div></div><div class="obody">`;
    ...
    h+=`<div class="gt blue"><span>${t('optionTotal')} B ${t('total')}${S.contOn?' (w/ '+S.contPct+'%)':''}</span><span>${fmt(T[1].t)}</span></div>`;
    h+=`</div></div></div>`;
    ...
    // SLIDE D
    h+=`<div class="csl"><div class="ocard"><div class="ohdr red"><div class="tr"><h2>${S.morgEq==='ford'?t('slideDf'):t('slideDs')}</h2><span class="pbadge red">${fmt(T[3].t)}</span></div></div><div class="obody">`;
    ...
    h+=`<div class="gt red"><span>${t('optionTotal')} D ${t('total')}${S.contOn?' (w/ '+S.contPct+'%)':''}</span><span>${fmt(T[3].t)}</span></div>`;
    h+=`</div></div></div>`;
...
function printReport(){
  ...
  const optData=[
    {t:T[0],c:'green',gcLabel:'GC: KFS \u2014 Layout 1',gcArr:D.a_gc,eqLabel:'Equipment: Ford Hotel Supply',eqArr:D.a_eq,eqKey:'a',notes:'KFS (est. 1924): MEP specialist. Includes demo, FRP, SS panels, hood trim, pot filler waterline, 6 LED, equipment set/connect. Excludes flooring, ceiling, permits, plans.'},
    {t:T[1],c:'blue',gcLabel:'GC: Pernikoff (Revised 3/9)',gcArr:D.b_gc,eqLabel:'Equipment: Session + Kitchen Mechanical',eqArr:D.b_eq,eqKey:'b',notes:'Pernikoff (est. 1914). Base: LED, 7 outlets, water heater, plumbing, demo, tile, SS panel install, infill door, paint ($1,500). Excludes appliances, faucets, SS panels, plans, permits.'},
    ...
    {t:T[3],c:'red',gcLabel:'GC: Morganco Design Build',gcArr:D.d_gc,eqLabel:'Equipment: '+(S.morgEq==='ford'?'Ford (Appliances)':'Session (Appliances)'),eqArr:S.morgEq==='ford'?D.d_ef:D.d_es,eqKey:S.morgEq==='ford'?'df':'ds',notes:'Morganco = Design-Build. Includes design (4 meetings + 3D), QualityBrand cabinetry, Hallmark quartz, FRP + SS, electrical, plumbing. Expires 3/27/2026.'},
  ];
